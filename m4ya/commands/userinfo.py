...
@apc.command(name="userinfo", description="Sends User Info",)
async def userinfo(interaction: discord.Interaction, user: discord.User,  ephemeral:bool=True):
    ...
    await interaction.response.send_message(f"Heres is <@{user.id}>'s info!", embed=e, ephemeral=ephemeral)

async def setup(bot: discord.Client):
    bot.tree.add_command(userinfo)

async def teardown(bot: discord.Client):
    bot.tree.remove_command(userinfo)